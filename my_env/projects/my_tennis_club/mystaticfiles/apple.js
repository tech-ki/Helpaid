...
let o = "";
  const dataArray = [
    ...
    for (let i in dataArray) {
      let g = i;
    o += "<h3 data-id="+g+" class='"+dataArray[i].class+"'>" + dataArray[i].name+"</h3>"+"{% for x in mymembers %}<h3 class='{{ x.cclass }}'>{{ x.item }} </h3> {% endfor %}" ;
}

/* "{% for x in mymembers %}<h3 class='{{ x.cclass }}'>{{ x.item }} </h3> {% endfor %}" */
